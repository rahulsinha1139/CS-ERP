...
const paginationSchema = zod_1.z.object({
    ...
    sortBy: zod_1.z.enum(['name', 'email', 'createdAt', 'totalBilled']).default('name'),
...
const createCustomerSchema = zod_1.z.object({
    ...
    creditDays: zod_1.z.number().min(0).optional(),
    ...
    registeredOffice: zod_1.z.string().optional(),
...
exports.customerRouter = (0, trpc_1.createTRPCRouter)({
    // Get all customers with financial summary - OPTIMIZED VERSION
    getAll: trpc_1.protectedProcedure // Apply security
        ...
        const enhancedCustomers = customers.map((customer) => {
            ...
            const overdueInvoices = customer.invoices.filter((inv) => inv.dueDate && new Date(inv.dueDate) < new Date() && inv.status !== 'PAID').length;
            return {
                id: customer.id,
                name: customer.name,
    ...
    getInvoices: trpc_1.protectedProcedure
        ...
        status: zod_1.z.enum(['ALL', 'PAID', 'UNPAID', 'OVERDUE', 'PARTIALLY_PAID']).default('ALL'),
    }))
        .query(async ({ ctx, input }) => {
        const whereClause = {
    ...
    getList: trpc_1.protectedProcedure
        .query(async ({ ctx }) => {
        if (!ctx.companyId) {
            throw new server_1.TRPCError({
                code: 'UNAUTHORIZED',
    ...
    create: trpc_1.protectedProcedure
        ...
        const customer = await ctx.db.customer.create({
            data: {
                id: id_generator_1.idGenerator.customer(), // ✅ UUID v4 - No race conditions
                ...input,
                companyId: ctx.companyId,
            },
        });
    ...
    getTopCustomers: trpc_1.protectedProcedure
        .input(zod_1.z.object({
        limit: zod_1.z.number().min(1).max(50).default(10),
        period: zod_1.z.enum(['7d', '30d', '90d', '1y', 'all']).default('all'),
    }))
        .query(async ({ ctx, input }) => {
        const dateFilter = {};
        const now = new Date();
    ...
    getAllWithSummary: trpc_1.protectedProcedure
        .input(paginationSchema.optional())
        .query(async ({ ctx, input = {} }) => {
        const { page = 1, limit = 20, search, sortBy, sortOrder } = input;
        const offset = (page - 1) * limit;
        ...
        const optimizedCustomers = customers.map((customer) => {
            ...
            const partiallyPaidInvoices = customer.invoices.filter(inv => inv.paidAmount > 0 && inv.paidAmount < inv.grandTotal).length;
            const unpaidInvoices = customer.invoices.filter(inv => inv.paidAmount === 0).length;
            // Overdue calculation
            const overdueInvoices = customer.invoices.filter(inv => inv.dueDate &&
                new Date(inv.dueDate) < new Date() &&
                inv.paidAmount < inv.grandTotal).length;
            // Performance metrics
