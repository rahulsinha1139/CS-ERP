...
describe('GST Engine', () => {
    describe('calculateGST', () => {
        test('should calculate CGST and SGST for intrastate transaction', () => {
            ...
            const result = gst_engine_1.gstEngine.calculateGST(input);
    ...
    describe('validateGSTIN', () => {
        test('should validate correct GSTIN format', () => {
            ...
            expect(gst_engine_1.gstUtils.validateGSTIN(validGSTIN)).toBe(true);
        });
        test('should reject invalid GSTIN format', () => {
            expect(gst_engine_1.gstUtils.validateGSTIN('INVALID')).toBe(false);
            expect(gst_engine_1.gstUtils.validateGSTIN('07AAACB0618C1')).toBe(false); // Too short
            expect(gst_engine_1.gstUtils.validateGSTIN('99AAACB0618C1Z5')).toBe(false); // Invalid state code
        });
        test('should handle empty GSTIN', () => {
            expect(gst_engine_1.gstUtils.validateGSTIN('')).toBe(false);
            expect(gst_engine_1.gstUtils.validateGSTIN(null)).toBe(false);
        });
    });
    describe('getStateName', () => {
        test('should return correct state info for valid code', () => {
            const delhiInfo = gst_engine_1.gstEngine.getStateName('07');
            expect(delhiInfo).toBe('Delhi');
            // State code is the key, so no need to test this
            const maharashtraInfo = gst_engine_1.gstEngine.getStateName('27');
            expect(maharashtraInfo).toBe('Maharashtra');
            // State code is the key, so no need to test this
        });
        test('should return null for invalid state code', () => {
            expect(gst_engine_1.gstEngine.getStateName('99')).toBeUndefined();
            expect(gst_engine_1.gstEngine.getStateName('ABC')).toBeUndefined();
        });
    });
    describe('Edge Cases', () => {
        ...
        test('should handle negative amounts gracefully', () => {
            ...
            expect(() => gst_engine_1.gstEngine.calculateGST(input)).toThrow();
        });
    });
});
