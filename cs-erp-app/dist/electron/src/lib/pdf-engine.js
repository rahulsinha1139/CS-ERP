...
exports.pdfEngine = exports.PDFEngine = void 0;
...
const generateStatusBadge = (data) => {
...
const generateHTMLInvoice = (data) => {
...
<body>
  <div class="container">
      ...
      <div class="billing-section">
        ...
        <div class="billing-column">
          ...
          <div class="customer-info">
            ...
            <p><strong>Issue Date:</strong> ${(0, utils_1.formatDate)(data.invoice.issueDate)}</p>
            ${data.invoice.dueDate ? `<p><strong>Due Date:</strong> ${(0, utils_1.formatDate)(data.invoice.dueDate)}</p>` : ''}
        ...
        <tbody>
          ${data.lineItems.map(item => `
            <tr>
              ...
              <td class="number">${(0, utils_1.formatCurrency)(item.rate)}</td>
              <td class="number">${(0, utils_1.formatCurrency)(item.taxableValue)}</td>
              ${!data.totals.isInterstate ?
        `<td class="number">${(0, utils_1.formatCurrency)(item.cgst)}</td><td class="number">${(0, utils_1.formatCurrency)(item.sgst)}</td>` :
        `<td class="number">${(0, utils_1.formatCurrency)(item.igst)}</td>`}
              <td class="number"><strong>${(0, utils_1.formatCurrency)(item.lineTotal)}</strong></td>
            </tr>
          `).join('')}
        </tbody>
      </table>
      ...
      <div class="totals-section">
        <table class="totals-table">
          <tr>
            ...
            <td class="value">${(0, utils_1.formatCurrency)(data.totals.subtotal)}</td>
          </tr>
          ${data.totals.totalDiscount && data.totals.totalDiscount > 0 ?
        `<tr><td class="label">Total Discount:</td><td class="value">-${(0, utils_1.formatCurrency)(data.totals.totalDiscount)}</td></tr>` : ''}
          ${data.totals.cgstAmount > 0 ?
        `<tr><td class="label">CGST:</td><td class="value">${(0, utils_1.formatCurrency)(data.totals.cgstAmount)}</td></tr>` : ''}
          ${data.totals.sgstAmount > 0 ?
        `<tr><td class="label">SGST:</td><td class="value">${(0, utils_1.formatCurrency)(data.totals.sgstAmount)}</td></tr>` : ''}
          ${data.totals.igstAmount > 0 ?
        `<tr><td class="label">IGST:</td><td class="value">${(0, utils_1.formatCurrency)(data.totals.igstAmount)}</td></tr>` : ''}
          ${data.totals.roundOff && data.totals.roundOff !== 0 ?
        `<tr><td class="label">Round Off:</td><td class="value">${data.totals.roundOff > 0 ? '+' : ''}${(0, utils_1.formatCurrency)(data.totals.roundOff)}</td></tr>` : ''}
          <tr class="grand-total">
            <td class="label"><strong>Grand Total:</strong></td>
            <td class="value"><strong>${(0, utils_1.formatCurrency)(data.totals.grandTotal)}</strong></td>
          </tr>
        </table>
      </div>
    ...
    <div class="footer">
      ...
      <p>Generated on: ${(0, utils_1.formatDate)(new Date())} | Powered by ${data.company.name}</p>
...
exports.pdfEngine = PDFEngine.getInstance();
