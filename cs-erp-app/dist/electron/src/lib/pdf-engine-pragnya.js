...
exports.pragnyaPDFEngine = exports.PragnyaPDFEngine = void 0;
const utils_1 = require("./utils");
// Helper function to render service-specific details table
const renderServiceDetailsTable = (item) => {
          ...
          <tbody>
            ${item.details.map((detail) => `
              <tr>
                ...
                ${firstDetail.govtFees !== undefined ? `<td>${(0, utils_1.formatCurrency)(detail.govtFees || 0)}</td>` : ''}
                <td>${(0, utils_1.formatCurrency)(detail.professionalFees || detail.amount)}</td>
              </tr>
            `).join('')}
            ${item.subtotals ? `
              <tr style="font-weight: 600; background-color: #f5f5f5;">
                ...
                ${firstDetail.govtFees !== undefined ? `<td>${(0, utils_1.formatCurrency)(item.subtotals.govtFees || 0)}</td>` : ''}
                <td>${(0, utils_1.formatCurrency)(item.subtotals.professionalFees || 0)}</td>
              </tr>
            ` : ''}
          </tbody>
        </table>`;
          ...
          <tbody>
            ${item.details.map((detail) => `
              <tr>
                ...
                <td>${(0, utils_1.formatCurrency)(detail.fee || detail.amount)}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>`;
          ...
          <tbody>
            ${item.details.map((detail) => `
              <tr>
                <td>${detail.period || '-'}</td>
                <td>${detail.hours || '-'}</td>
                <td>${(0, utils_1.formatCurrency)(detail.ratePerHour || 0)}</td>
          ...
          <tbody>
            ${item.details.map((detail) => `
              <tr>
                ...
                ${firstDetail.srn !== undefined ? `<td>${detail.srn || '-'}</td>` : ''}
                <td>${(0, utils_1.formatCurrency)(detail.amount || 0)}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>`;
...
const generatePragnyaHTML = (data) => {
    return `<!DOCTYPE html>
<html>
<head>
...
<body>
  <div class="container">
    ...
    <div class="invoice-row">
      ...
      <div class="invoice-date">Date: ${(0, utils_1.formatDate)(data.invoice.issueDate)}</div>
    </div>
      ...
      <tbody>
        ${data.lineItems.map((item, index) => `
          <tr>
            ...
            <td class="number">${(0, utils_1.formatCurrency)(item.amount)}</td>
          </tr>
        `).join('')}
        <tr class="total-row">
          ...
          <td class="number"><strong>${(0, utils_1.formatCurrency)(data.totals.grandTotal)}</strong></td>
        </tr>
      </tbody>
    </table>
...
exports.PragnyaPDFEngine = PragnyaPDFEngine;
exports.pragnyaPDFEngine = new PragnyaPDFEngine();
