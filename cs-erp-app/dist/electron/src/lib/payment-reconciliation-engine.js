...
exports.paymentUtils = exports.paymentReconciliationEngine = exports.PaymentReconciliationEngine = exports.ReconciliationStatus = exports.PaymentMethod = exports.PaymentStatus = void 0;
...
class PaymentReconciliationEngine {
     ...
     */
    async reconcilePayments(payments, invoices, options) {
     ...
     */
    async reconcileBankStatement(bankEntries, payments, options) {
        ...
        if (options?.dateRange) {
            filteredBankEntries = bankEntries.filter(entry => entry.date >= options.dateRange.start && entry.date <= options.dateRange.end);
        }
        // Match bank entries to payments
        for (const bankEntry of filteredBankEntries) {
            if (bankEntry.type !== 'CREDIT')
                continue; // Only match credits for payments
            // Look for exact amount matches
            const amountMatches = payments.filter(payment => Math.abs(payment.amount - bankEntry.amount) <= tolerance);
            if (amountMatches.length === 1) {
                ...
                if (bankIndex > -1)
                    unmatchedBankEntries.splice(bankIndex, 1);
                const paymentIndex = unmatchedPayments.findIndex(p => p.id === payment.id);
                if (paymentIndex > -1)
                    unmatchedPayments.splice(paymentIndex, 1);
            }
     ...
     */
    async generatePaymentAnalytics(payments, invoices, period) {
        let filteredPayments = payments;
        let filteredInvoices = invoices;
        if (period) {
            filteredPayments = payments.filter(p => p.paymentDate >= period.start && p.paymentDate <= period.end);
            filteredInvoices = invoices.filter(inv => inv.dueDate && inv.dueDate >= period.start && inv.dueDate <= period.end);
        }
        ...
        for (const payment of filteredPayments) {
            if (payment.invoiceNumber) {
                ...
                if (invoice && invoice.dueDate) {
                    const daysDiff = Math.floor((payment.paymentDate.getTime() - invoice.dueDate.getTime()) / (1000 * 60 * 60 * 24));
        ...
        // Overdue analysis
        const now = new Date();
        const overdueInvoices = filteredInvoices.filter(inv => inv.dueDate && inv.dueDate < now && inv.remainingAmount > 0);
     ...
     */
    async predictPaymentProbability(invoiceId, historicalData) {
    ...
    findExactMatch(payment, invoices, tolerance) {
    ...
    findPartialMatch(payment, invoices, tolerance) {
        for (const invoice of invoices) {
    ...
    async findFuzzyMatch(payment, invoices, _tolerance) {
        const matches = [];
        for (const invoice of invoices) {
            let score = 0;
            let notes = '';
            ...
            if (payment.reference && invoice.number) {
                const refSimilarity = this.calculateStringSimilarity(payment.reference.toLowerCase().trim(), invoice.number.toLowerCase().trim());
            ...
            if (payment.customerName && invoice.customerName) {
                const nameSimilarity = this.calculateStringSimilarity(payment.customerName.toLowerCase().trim(), invoice.customerName.toLowerCase().trim());
    ...
    checkForDuplicates(payment, allPayments, _processedIds) {
        ...
        const duplicates = allPayments.filter(p => p.id !== payment.id && // Don't compare with itself
    ...
    findBestBankMatch(bankEntry, paymentMatches) {
        let bestMatch = null;
        let bestScore = 0;
    ...
    calculateMonthlyTrends(payments) {
    ...
    calculateRecentBehaviorImpact(payments) {
...
exports.PaymentReconciliationEngine = PaymentReconciliationEngine;
// Export singleton instance
exports.paymentReconciliationEngine = PaymentReconciliationEngine.getInstance();
