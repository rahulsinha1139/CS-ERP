...
interface Invoice {
  ...
  client: string;
  amount: number;
  status: 'pending' | 'paid' | 'overdue' | 'draft' | 'partially-paid' | 'generated';
  dueDate?: string;
  paidDate?: string;
  isOverdue: boolean;
}
...
const useDashboardData = () => {
  ...
  const data = loading ? null : {
    metrics: dashboardMetrics || {
      revenue: { total: 0, quarterly: 0, received: 0, outstanding: 0, overdue: 0, collectionRate: 0 },
    ...
    recentInvoices: (recentInvoicesData?.invoices || []).map((invoice: ApiInvoice) => ({
      ...
      status: ({
        ...
        'OVERDUE': 'overdue',
        'CANCELLED': 'draft'
      }[invoice.status] || 'draft') as 'pending' | 'paid' | 'overdue' | 'draft' | 'partially-paid' | 'generated',
...
const QuickActions = () => {
  ...
  const handleInvoiceGroups = () => router.push('/invoice-groups');
      ...
      <AuraCardContent>
        <AuraGrid cols={2} gap="sm">
          ...
          <AuraButton
            ...
            size="lg"
            icon={<FolderIcon />}
            onClick={handleInvoiceGroups}
            className="h-auto py-4 px-3 flex-col items-start text-left w-full justify-start"
          >
            <span className="font-medium text-sm whitespace-nowrap">Invoice Groups</span>
            <span className="text-xs opacity-80 whitespace-nowrap">Quarterly invoicing</span>
          </AuraButton>
        </AuraGrid>
      </AuraCardContent>
    </AuraCard>
    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <AuraGrid cols={4} gap="lg">
          ...
          <div onClick={() => router.push('/compliance')} className="cursor-pointer">
            <AuraMetricCard
