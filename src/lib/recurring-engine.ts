...
export interface RecurringContract {
  ...
  // Customer and service info (for processing)
  customer?: {
    id: string;
    name: string;
    email?: string;
...
export class RecurringEngine {
  private static instance: RecurringEngine;

  static getInstance(): RecurringEngine {
   ...
   */
  calculateNextBillingDate(currentDate: Date, frequency: BillingFrequency): Date {
    switch (frequency) {
   ...
   */
  calculateEscalation(contract: RecurringContract, effectiveDate: Date = new Date()): EscalationCalculation {
  ...
  async processRecurringBilling(
    ...
    processed: Array<{ contractId: string; invoiceId: string; success: boolean; error?: string }>;
    summary: {
   ...
   */
  isBillingDue(contract: RecurringContract, checkDate: Date = new Date()): boolean {
   ...
   */
  updateContractAfterBilling(contract: RecurringContract, billingDate: Date): RecurringContract {
   ...
   */
  private generateInvoiceFromContract(
    contract: RecurringContract,
    escalation: EscalationCalculation,
    billingDate: Date
  ): InvoiceData {
    ...
    const invoiceNumber = invoiceEngine.generateInvoiceNumber('REC', billingDate.getFullYear());
   ...
   */
  private calculateYearsElapsed(startDate: Date, currentDate: Date): number {
  ...
  calculateRevenueForecast(
    ...
    monthly: Array<{ month: string; amount: number; contractCount: number }>;
    total: number;
    averageMonthly: number;
  } {
    const monthlyData = new Map<string, { amount: number; contractCount: Set<string> }>();
    let totalRevenue = 0;
    ...
    for (const contract of contracts) {
      ...
      for (const billing of schedule) {
        ...
        const existing = monthlyData.get(monthKey) || { amount: 0, contractCount: new Set<string>() };
   ...
   */
  validateContract(contract: Partial<RecurringContract>): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];

    if (!contract.customerId) errors.push('Customer ID is required');
    if (!contract.serviceTemplateId) errors.push('Service template ID is required');
    if (!contract.amount || contract.amount <= 0) errors.push('Amount must be positive');
    if (!contract.frequency) errors.push('Billing frequency is required');
...
export const recurringEngine = RecurringEngine.getInstance();
...
export const recurringUtils = {
   ...
   */
  formatFrequency: (frequency: BillingFrequency): string => {
    const map = {
   ...
   */
  getFrequencyMultiplier: (frequency: BillingFrequency): number => {
    const map = {
   ...
   */
  calculateAnnualValue: (amount: number, frequency: BillingFrequency): number => {
    const multiplier = recurringUtils.getFrequencyMultiplier(frequency);
   ...
   */
  getNextBillingDescription: (contract: RecurringContract): string => {
    const nextDate = format(contract.nextBillingDate, 'dd MMM yyyy');
    const escalation = recurringEngine.calculateEscalation(contract);
   ...
   */
  needsAttention: (contract: RecurringContract): { needs: boolean; reasons: string[] } => {
    ...
    if (recurringEngine.isBillingDue(contract)) {
