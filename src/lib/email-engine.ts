...
export class EmailEngine {
  ...
  async sendPaymentReminder(
    customerEmail: string,
    template: InvoiceEmailTemplate & { daysOverdue: number }
  ): Promise<EmailDeliveryResult> {
   ...
   */
  private generateInvoiceEmailHTML(template: InvoiceEmailTemplate): string {
...
<body>
   ...
   */
  private generateInvoiceEmailText(template: InvoiceEmailTemplate): string {
    const formattedAmount = new Intl.NumberFormat('en-IN', {
      style: 'currency',
      currency: 'INR',
    }).format(template.amount);
...
This is an automated email. Please do not reply directly to this message.`;
   ...
   */
  private generateReminderEmailHTML(template: InvoiceEmailTemplate & { daysOverdue: number }): string {
    const formattedAmount = new Intl.NumberFormat('en-IN', {
      style: 'currency',
      currency: 'INR',
    }).format(template.amount);
...
<body>
   ...
   */
  private generateReminderEmailText(template: InvoiceEmailTemplate & { daysOverdue: number }): string {
    const formattedAmount = new Intl.NumberFormat('en-IN', {
      style: 'currency',
      currency: 'INR',
    }).format(template.amount);
...
This is an automated reminder. Please contact us if you need assistance.`;
   ...
   */
  private generateComplianceReminderHTML(title: string, dueDate: Date, companyName: string): string {
...
<body>
   ...
   */
  private generateComplianceReminderText(title: string, dueDate: Date, companyName: string): string {
    const formattedDate = new Intl.DateTimeFormat('en-IN').format(dueDate);
...
export const emailUtils = {
   ...
   */
  checkDeliverability: (email: string): { valid: boolean; reason?: string } => {
